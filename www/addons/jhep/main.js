...
.config(function($stateProvider, $mmSideMenuDelegateProvider, mmaJhepPriority) {
    ...
    $stateProvider
        ...
        .state('site.jhep-event', {
            url: '/jhep-event/:id', // We need to add ID to the URL to make ng-href work.
            views: {
                'site': {
                    controller: 'mmaCalendarEventCtrl',
                    templateUrl: 'addons/jhep/templates/event.html'
                }
            }
        });
    ...
    $mmLocalNotifications.registerClick(mmaCalendarComponent, function(data) {
        if (data.eventid) {
            $mmApp.ready().then(function() {
                $state.go('redirect', {siteid: data.siteid, state: 'site.jhep', params: {eventid: data.eventid}});
            });
        }
    });
